...
def executeStep(String GITHUB_REPO_NAME) {

    //This is to ensure the global environment variables get set on the parallel executing container environments
    env.ASOC_APPLICATION_ID="${ASOC_APPLICATION_ID}"
    env.GITHUB_ORG_NAME="${GITHUB_ORG_NAME}"
    env.ART_URL="${ART_URL}"
    env.JENKINS_ART_CREDENTIALS="${JENKINS_ART_CREDENTIALS}"
    env.GITHUB_ISSUE_BOARD="${GITHUB_ISSUE_BOARD}"
